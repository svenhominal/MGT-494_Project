# Replication Project: Pass-through of Emissions Costs in Electricity Markets
...
This repository contains a replication and extension of the original paper's analysis examining how EU-ETS carbon prices affected Spanish electricity market prices during 2004-2007.
...
├── calcul.py                           # Calculation utilities
├── init.py                             # Initialization script
│
├── replication_notebook.ipynb          # Main replication analysis in Python
├── replication_extension_notebook.ipynb # Extensions to the original analysis
├── extension_ideas.ipynb               # Ideas for further extensions
├── ml_causal_forest.ipynb              # Machine learning extension (causal forests)
│
├── Initial code/                       # Original replication code package
│   ├── stata_code/                     # Stata analysis scripts
│   ├── stata_data/                     # Processed Stata datasets
│   ├── matlab_code/                    # MATLAB market clearing algorithms
│   ├── matlab_data/                    # Daily bidding data (CSV)
│   ├── figures/                        # Output figures (EPS/PDF)
│   ├── tables/                         # Output tables (LaTeX)
│   └── original_figures/               # Original paper figures
│
└── python_translation/                 # Python translations of original code
    ├── python_code/                    # Translated Python scripts
    ├── stata_data/                     # Data for Python analysis
...
### Python Notebooks (Root Directory)

| Notebook | Purpose |
|----------|---------|
| `replication_notebook.ipynb` | Full Python replication of main results |
| `replication_extension_notebook.ipynb` | Extensions beyond original paper |
| `ml_causal_forest.ipynb` | Causal forest ML extension |
| `extension_ideas.ipynb` | Research extension ideas |
...
### For Stata Analysis
- Stata 13+ (original code)
- Access to raw data (not included in public package)

### For MATLAB Analysis
- MATLAB R2012+ 
- IBM CPLEX solver (academic license available)

### For Python Replication
```
pandas
numpy
scipy
...
seaborn
statsmodels
linearmodels (for IV/panel regressions)
```
...
### Option 1: Python Replication (Recommended)
1. Open `replication_notebook.ipynb`
2. Update the `DIRPATH` variable to point to your data location
3. Run all cells sequentially

### Option 2: Original Stata/MATLAB Pipeline
1. Edit `master_run_code.do` with correct `$datapath` and `$dirpath`
2. Run in Stata: `do master_run_code.do`
3. For elasticity analysis: Run `elasticityMarkupAnalysis.m` in MATLAB
...
|----------|-------------|--------|
| `price` / `pbid` | Electricity bid/clearing price (€/MWh) | Market operator |
| `eua_price` | EU Allowance price (€/ton CO2) | Carbon market |
| `erate` | Emission rate (tons CO2/MWh) | Technical data |
| `mg_cost` | Marginal cost = fuel cost + erate × CO2 price | Computed |
| `mwh` / `qbid` | Quantity bid/produced (MWh) | Market operator |
| `firm_code` | Anonymized firm identifier | Confidential |
| `hour` | Hour of day (1-24) | Time dimension |

---

## 📝 Notes

- **Data confidentiality**: Raw bidding data is confidential; only processed datasets and samples are included
- **Sample period**: Main analysis covers 2004-2007; MATLAB sample is June 2005
- **Market context**: Spanish electricity market (OMEL) day-ahead auction
...
See sections above for detailed running instructions.